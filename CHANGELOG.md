...
- **CrumpleVideo / Kaggle SaveKernel CPU Fallback**: the same shared Kaggle client path now also covers `CrumpleVideo`: `/v tomorrow` still requests GPU first, treats non-empty `ApiSaveKernelResponse.error` as a failed push instead of a false success, and retries once on CPU on the same `save_kernel` path when Kaggle rejects the launch with the weekly GPU quota error.
...
- **CherryFlash / Story-Native Final Artifact**: CherryFlash final mode now renders directly to `720x1280` and encodes the single publish artifact as `H.264/AAC` (`libx264`, `crf=26`, `preset=fast`, `profile=high`, `level=4.1`, `+faststart`, `128k audio`) instead of building an FHD/HEVC master and hoping downstream story delivery will shrink it acceptably.
- **CherryFlash / Story-Safe Shared Upload Contract**: the shared Kaggle `story_publish.py` helper now targets the same `720x1280` Telegram-story canvas and continues to normalize story uploads into `H.264/AAC` before `SendStoryRequest`, preventing `MEDIA_FILE_INVALID` without creating a separate channel-side publish artifact.
