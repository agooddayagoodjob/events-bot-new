...
- **Guide Excursions / Soft-Partial Digest Autopublish Guard**: scheduled and recovery guide digest auto-publish no longer abort just because monitor import recorded the synthetic warning `kaggle result marked as partial`; hard monitor failures still block publish, but a soft-partial run that already materialized usable candidates can now continue the daily digest path instead of dropping the whole issue.
- **Guide Excursions / Dense Multi-Announce Stage Split**: the Kaggle guide runner now uses a thinner `trail_scout.screen.v1` for routing, a dedicated `trail_scout.block_router.v1` for dense schedule cards, slimmer block-extract schemas without nested `fact_pack/template_hint/profile_hint`, and conditional `route_weaver.enrich.v1` calls only when semantic fill is still needed, reducing LLM load on posts like `vkaliningrade/4661`.
