...
- CherryFlash: corrected the handoff contract so scene `1` no longer starts too deep inside `move_up`, and raised/restyled the phone-screen CTA toward a more app-like Cygre UI lockup above the ribbon.
- CherryFlash: moved the active intro screen contract toward a premium app-like dark phone UI and late-3D tonal transition instead of relying on the early 2D tail for the first white-to-black shift.
- **Guide Excursions / Grounded Guide Identity Guard**: non-personal guide sources no longer auto-publish `guide_profile.guide_line` or fallback source names as `👤 Гид` unless the same person is confirmed in occurrence-level `guide_names` or in the grounded excerpt of the concrete post; otherwise digest/editorial now fall back to `🏢 Организатор` or hide the line, preventing incidents like `ruin_keepers/5172`.
...
- **CrumpleVideo / Story Required Health Guard**: production can now declare `/v` stories mandatory with `VIDEO_ANNOUNCE_STORY_REQUIRED=1`; `/healthz` fails closed when story publish is silently disabled or obviously misconfigured, so stale branch/config deploys stop looking healthy while dropping Telegram stories.
- **Reports / Incident Index**: added a canonical `docs/reports/incidents/` index and recorded the April 10, 2026 CrumpleVideo story regression caused by branch/config drift.
- **Operations / Runtime Log Retention**: production can now mirror the existing root logger into hourly-rotated files under `/data/runtime_logs`, keeping roughly 24 hours of on-machine logs for incident review without growing unbounded; the retention is controlled by `ENABLE_RUNTIME_FILE_LOGGING`, `RUNTIME_LOG_DIR`, `RUNTIME_LOG_BASENAME`, and `RUNTIME_LOG_RETENTION_HOURS`.
- **Guide Excursions / Media Caption Period Line**: guide digest media albums now show a compact date-period line in the caption (`12 апреля`, `12, 14 и 16 апреля`, `11-15 апреля`) before the down-links, so the album itself tells the reader which dates the found excursions cover without bloating the caption.
- **CherryFlash / Kaggle Intro+Scene1 Preproduction Path**: added a dedicated `kaggle/CherryFlash/` notebook/kernel path plus `kaggle/execute_cherryflash_intro_scene1.py`, so the current clean render validation for `intro + first 2D scene + music` can run in Kaggle preproduction instead of depending on the local workstation render budget.
- **CherryFlash / Kaggle Asset Bundle**: CherryFlash now carries its own checked-in preproduction asset bundle under `kaggle/CherryFlash/assets/` for the phone model, Cygre font zip, compact music cue, and poster set needed by the current `intro + scene1` approval artifact.
- **CrumpleVideo / MobileFeed Intro Motion-Proof Refresh**: the `Мобильная лента` preview path now uses the user-provided `РО Знание` font pack for phone-screen CTA, unlit screen-locked label planes, stricter ribbon geometry guardrails for magazine-like paper behavior without false poster pinching, a larger hero-phone framing on an infinite-white floor setup, and sparse-keyframe preview rendering to validate the expensive handoff motion before another clean final pass.
- **CrumpleVideo / MobileFeed Screen-Fit + CTA Pass**: the active `Мобильная лента` still/preview path now moves the ribbon exit shoulders closer to the visible phone screen so the handoff poster reads as the true screen-width anchor, enlarges the phone-surface CTA toward the reference scale, and adds explicit canon/doc guardrails against falsely squeezed neighboring posters.

### Changed
- **CherryFlash / Motion-Curve + Tonal Handoff Retune**: the current `intro + scene1` approval path now drives the white-background 3D push-in with one continuous global motion curve and a more explicit premium ease instead of stitched timing plateaus or a near-linear push, renders preview cadence at full per-frame 3D cadence even in cheap mode, shifts scene-1 local start to `~2.09s` so the first upward move lands slightly earlier against the strong `Pulsarium` accent, replaces the old hybrid handoff strategy with a true cut plus a milky-to-black tonal fade inside the early 2D frames, and switches preview encoding to CRF-based x264 so motion-proof clips stay much lighter without reverting to fake interpolation.
- **CherryFlash / Late White Push Stabilization**: the intro camera no longer blends two competing location solvers in the close-in white phase; late approach frames now use one monotonic location model, which removes the last visible flutter cluster around the pre-handoff push while keeping the brighter screen treatment and gradual milky-to-dark 2D tail.
- **CherryFlash / Final Intro Render Contract**: the `intro + scene1` approval renderer now treats `--final` as a real clean path instead of a dressed-up preview: final mode renders every intro frame at full cadence (`frame_step=1`), forbids synthetic in-between intro frames from `Image.blend`, raises the Cycles quality budget, enables GPU usage for final Kaggle renders, removes the visible whole-frame hybrid dissolve from the final handoff path, adds a raw-frame reuse path for fast local final rebuilds, and nudges scene-1 local start from `~1.82s` to `~1.95s` so the upward move is already more decisively underway before the strongest early `Pulsarium` accent.
- **CherryFlash / Kaggle Preproduction Bootstrap**: the CherryFlash preproduction path now uses a narrower runtime bootstrap for `zigomaro/cherryflash`: the notebook first follows the production-style mounted-bundle lookup under `/kaggle/input`, and only if that is absent falls back to cloning the already-pushed CherryFlash branch head so the team can validate the clean `intro + first 2D scene + music` artifact without being blocked by flaky Kaggle dataset propagation.
- **CherryFlash / Canonical Render Path Cleanup**: the CherryFlash spec and design brief now point to the dedicated `kaggle/CherryFlash/` notebook, Blender helper, and Kaggle execution script as the active approval/preproduction route, removing the leftover historical implication that CherryFlash approval renders still live under `kaggle/CrumpleVideo/`.
- **CherryFlash / Kaggle Notebook Source Resolution**: the CherryFlash notebook now prints visible `/kaggle/input` directories for diagnosis and resolves the working render bundle by the actual `scripts/render_mobilefeed_intro_scene1_approval.py` path instead of assuming one exact mounted folder name.
- **CherryFlash / Feature-Owned Asset Resolution**: `scripts/render_mobilefeed_intro_still.py` now prefers feature-owned assets under `kaggle/CherryFlash/assets/` before older local reference/artifact paths, keeping the CherryFlash preproduction route self-owned.
- **Docs / CherryFlash Preproduction Rollout**: the CherryFlash canon now records the current delivery sequence explicitly: `Kaggle preproduction` first on `intro + first 2D scene + music`, then a full `2..6` scene validation run, and only after successful testing the future production gate may enable story autopublish reused from the proven CrumpleVideo story path, with the target operating expectation that the story should already be live by local noon.
- **Docs / CherryFlash Canonical Path**: the popularity-driven daily video product now uses `CherryFlash` as the canonical documentation path and product name, with the main spec moved to `docs/backlog/features/cherryflash/README.md` and the design brief moved to `docs/backlog/features/cherryflash/design-brief.md`; the old `crumple-video-popular-review` canonical doc path has been removed from active routing, while `popular_review` stays only as a technical internal alias.
- **Docs / CherryFlash Product Naming + Runtime Separation**: the popularity-driven `/v -> Быстрый обзор` backlog spec is now explicitly branded as `CherryFlash / Popular Video Afisha` with internal mode key `popular_review`, and the canon now states unambiguously that this is a separate daily product which must not modify or destabilize the existing `CrumpleVideo` runtime even if some temporary Blender approval scripts still live under historical `kaggle/CrumpleVideo/` paths.
- **CrumpleVideo / MobileFeed CTA Routing Refresh**: the active `Мобильная лента` phone-screen CTA now shifts from `count + month` to `count + real type cluster` (for the current selection, `6 событий` plus `кино • лекции • встречи • экскурсии`), while the lower phone label and the quiet bottom city treatment stay on `Cygre` for a more coherent UI-like screen layer.
- **CrumpleVideo / MobileFeed Motion-Approval Preview Retune**: `scripts/render_mobilefeed_intro_scene1_approval.py` now targets a cheaper half-resolution `540x960` motion-proof clip under `artifacts/codex/mobilefeed_intro_scene1_preview/`, with a slower combined opening move, a deeper late-tail cut, a tighter late whole-frame pre-cut dissolve around `104-106` instead of a poster-only hybrid composite, and scene-1 local start shifted to `~1.82s` so the first upward move is already visibly underway before the strongest early `Pulsarium` accent instead of lagging behind it; the preview assembly also dropped optical-flow interpolation in favor of sparse-keyframe frame blending to remove the previous doubled-edge phone-contour artifact, while phone-screen CTA now exits later so it dies closer to the transition instead of disappearing too early on white. The Blender tail-rerender path now also keeps absolute start-keyframes when densifying frames `96-106`, preventing the previous mid-clip jump back into an earlier wide phone pose, and the tighter late-dissolve avoids the mid-clip hybrid phone/poster parasite that still read as a déjà-vu insertion.
- **CrumpleVideo / MobileFeed Deep Lookdev Pass**: full tonal overhaul against the strict product-shot reference. AgX switched to `High Contrast` with exposure `-0.22` to fix the washed-out/overlit midtones. Key light reduced to 5200 (was 6200 which over-spilled the backdrop). Phone body changed to satin titanium (`roughness 0.18`, `coat 0.14`) so the dark base color reads instead of reflecting bright environment. Ribbon material stripped of glossy coat (`0.38→0.08`), emission near-zero (`0.005`), roughness `0.24` for printed-paper look; ribbon `visible_shadow` re-enabled so it casts a natural unified shadow from the key light together with the phone (no "two-source" artifact). Atlas color/contrast tuned to `1.44/1.18` (avoids crushing dark reds that collapsed to black in an earlier over-contrasty pass). Contact shadow reworked — offset `0.31→0.06` so the core sits at the phone's contact edge (not floating), texture rebuilt with asymmetric contact-line geometry. Frame metal sharper (`roughness 0.15`, `specular 0.78`); glass island denser (`fac 0.16`, coat 0.58); edge light at 860 for contour rim; glossy bounces 3; screen emission 0.22.
- **CrumpleVideo / MobileFeed Opus Lookdev Pass**: after a strict visual consultation against the phone+ribbon reference, the still path now uses a more contrasty AgX setup, lower ambient/HDRI fill, a stronger narrower key, a thinner brighter rim, denser contact-shadow treatment, brighter cooler screen emission, darker near-black island handling, and more product-shot metal/glass response to reduce the previous washed-out/plastic read.
- **CrumpleVideo / MobileFeed Screen-Light Correction**: corrected the previous over-darkened phone face so the screen stays bright and cool again without an invented black screen border, while the sensor-island remains near-black and the upper phone contour keeps stronger contrast against the background.
- **CrumpleVideo / MobileFeed Dark-Bezel + Stronger Shadow Pass**: the still render path now darkens the visible front phone face with a blacker bezel/sensor-island treatment, trims overfilling so the upper contour no longer washes toward gray, and strengthens the phone+ribbon shadow into a clearer contact core plus more readable down-right tail.
- **CrumpleVideo / MobileFeed Reference Light Hierarchy Pass**: the still render path now follows the strict phone+ribbon reference more literally with a dominant warm upper-left key, much weaker fill, a cooler rim/spec edge on the phone shell, brighter cooler screen treatment, denser poster color preprocessing, and a longer down-right contact-shadow tail so the product-shot depth no longer depends on flat ambient fill.
- **CrumpleVideo / MobileFeed World-Space CTA + Ribbon Density Pass**: the `Мобильная лента` still path now sizes phone-screen CTA from the real imported screen geometry in world space, raises the labels into the visible above/below bands instead of leaving them miniature over the poster, reshapes ribbon tails to keep neighboring poster widths from collapsing into rubber-like shoulder compression, and deepens poster density/material separation so the ribbon reads closer to printed glossy paper than to a pale dusty strip.
- **CrumpleVideo / MobileFeed Notch-Safe Cygre + Shadow Pass**: the still path now uses wider `Cygre` UI weights from the supplied archive for the phone CTA, keeps the top label out of the sensor/island area, restores a broader paper fall from the phone edge toward the surface instead of an overly flat tail, and retunes the key/fill/ambient balance so phone+ribbon shadows read closer to the strict product-shot reference while saturated poster reds stay denser.
- **CrumpleVideo / MobileFeed Asymmetrical Screen-Lockup**: the phone-screen CTA layout now prefers an art-directed left-column lockup under the sensor safe zone instead of a centered strip, with a cleaner `count + period` hierarchy in `Cygre` and a calmer lower support line that stays clear of the ribbon band.
- **CrumpleVideo / MobileFeed Master-Panel Scaling**: the `Мобильная лента` ribbon-fit contract now explicitly treats the second poster as the master panel, matches that poster to the visible phone width in the shot grammar, derives the shared ribbon height from that master poster, and keeps every other poster at native proportion under the same ribbon height.
- **CrumpleVideo / MobileFeed Reference Decomposition**: the strict `Мобильная лента` canon now explicitly locks the reference-specific ribbon behavior inside the phone area, including a sufficiently planar master-poster zone, broad edge bends starting at the phone boundary, in-front-of-screen ribbon layering, and off-frame ribbon continuation on both sides.
- **CrumpleVideo / MobileFeed Lookdev Guardrails**: the canon now treats the local Pro Max phone asset as sufficient and moves the current visual defect focus onto material/light/shading reproduction, requiring a warm-milky environment, a cooler cleaner screen, a darker premium phone shell, and denser non-chalky poster rendering.
- **CrumpleVideo / MobileFeed UI CTA + Edge-Shelf Pass**: the still path now moves phone CTA toward mixed-case UI typography above/below the ribbon and reshapes the ribbon edge exits into broader shelf-like bends so neighboring posters keep readable paper-like behavior near the phone boundary.
- **Guide Excursions / Multi-Target Digest Publish**: guide digest publish can now fan out the same media+text payload to every channel listed in `GUIDE_DIGEST_TARGET_CHATS`, while storing per-target message ids in `guide_digest_issue.published_targets_json`; historical backfill now copies Telegram albums as real album groups and rewires their caption links to the copied digest posts in the destination channel instead of pointing back to the source channel.
...
- **Guide Excursions / Jeep Tour Source + Terminology Guardrail**: added `@jeeptours39` to the canonical guide monitoring seed as a branded off-road source and taught the guide runtime/docs to preserve `джип-тур` / neutral `выезд` terminology instead of relabeling such announcements as generic excursions.
...
### Fixed
- **CrumpleVideo / Optional Telegram Poster Cache**: Kaggle render no longer depends on Telegram secrets just to prepare poster assets; Telegram lookup remains a best-effort cache fallback, and cache files now use hashed safe filenames so long Telegram URL tails cannot abort the run with `OSError: [Errno 36] File name too long` before normal HTTP poster download starts.
...
- **CrumpleVideo / Audio Source Contract**: server-side Kaggle session dataset assembly for `/v` now ships only `The_xx_-_Intro.mp3` for both test and prod runs, matching the notebook contract at `1:17` and preventing legacy `Pulsarium.mp3` from leaking back into final CrumpleVideo renders.
- **Telegram Monitoring / Malformed Festival Payloads**: server import now normalizes non-string `festival` values from `telegram_results.json` to `None`, and `smart_event_update` diagnostic title clipping no longer crashes on boolean payload fields, so a malformed Kaggle event no longer turns `tg_monitoring` recovery/scheduled imports into `partial`.
- **Scheduler / Critical TG+VK Run Guarantee**: `tg_monitoring` and `vk_auto_import` now use a longer per-job misfire window, default to waiting behind the heavy-job gate, and have startup/live catch-up checks that compare the last due slot against `ops_run`; if APScheduler emits `JOB_SUBMITTED`/`JOB_MISSED` without the entrypoint materializing a run, the bot dispatches the same scheduled path with a catch-up `run_id` instead of silently waiting for the next slot.
- **Guide Excursions / Recovery Import+Publish Durability**: guide monitoring now persists the downloaded Kaggle results bundle under `GUIDE_MONITORING_RESULTS_STORE_ROOT`, records `results_path` in the shared recovery registry, resumes import directly from that saved bundle after restarts, and only clears the recovery job after the scheduled `full` auto-publish path has either finished or definitively failed, so a restart between `Kaggle COMPLETE` and server import/digest publish no longer silently loses the run.
- **Guide Excursions Monitoring / Scheduled Bootstrap Visibility**: scheduled guide runs that fail before entering the main scan path now still write `ops_run(kind='guide_monitoring', status='error')` with `transport=bootstrap_error` and notify the admin chat, instead of disappearing as a silent “missed” slot; guide Kaggle numeric env parsing is also blank-safe now, so empty `GUIDE_MONITORING_*` overrides no longer kill the guide import chain at scheduler runtime.
- **CherryFlash / Kaggle MoviePy Compatibility**: `scripts/render_mobilefeed_intro_scene1_approval.py` now falls back to the `moviepy.audio.io` / `moviepy.video.io` import paths when Kaggle's bundled `moviepy` package does not expose `AudioFileClip` and `ImageSequenceClip` from the top-level module, and it no longer hard-depends on `AudioFadeOut` / `MultiplyVolume` module imports that are missing in Kaggle's packaged layout, unblocking the remote `intro + first 2D scene + music` render path.
...
- **CrumpleVideo / Kaggle GPU Metadata Guard**: the repo `CrumpleVideo` kernel metadata now requests GPU again, and the local-kernel deploy path force-keeps `enable_gpu=true` for `CrumpleVideo` pushes so a stale metadata drift cannot silently send tomorrow-runs back to `Accelerator None`.
