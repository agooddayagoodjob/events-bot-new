...
- **CherryFlash / 2D Primary Hold Cadence**: the full CherryFlash renderer no longer uses the problematic micro-drift after the main `move-up` in primary 2D scenes; the poster now holds steady until the exit phase, avoiding the “effective 15 fps / every-second-frame” cadence that remained perceptually rough even after exact duplicate hashes were removed.
