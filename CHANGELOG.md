...
- **Media Storage / Catbox Backfill Tooling**: added `scripts/backfill_catbox_posters_to_yandex.py` to re-fetch Telegram/VK source posters and persist Yandex-backed URLs into legacy `eventposter.supabase_*` + `event.photo_urls`, plus `scripts/inspect/audit_event_image_hosts.py` for statistical rollout verification by source and host class.
- **Media Storage / Future-Only Catbox Backfill Window**: `scripts/backfill_catbox_posters_to_yandex.py` now supports `--future-only` and `--from-date YYYY-MM-DD`, so prod backfills can rewrite only current/future events without touching historical cards.
- **Media Storage / Local-Heavy Backfill Plans**: catbox backfill can now emit reusable JSON plans via `--plan-out`, and `scripts/apply_backfill_plan.py` applies those patches directly to SQLite plus `telegraph_build` queue entries, so the expensive fetch/upload stage can run locally while prod only receives the lightweight DB update.
...
- **CherryFlash / Two-Step Kaggle Dataset Upload**: CherryFlash now creates each unique `cherryflash-session-*` dataset through a tiny bootstrap `CreateDataset` step followed by `CreateDatasetVersion` for the full runtime bundle, avoiding the Kaggle `CreateDataset` failure mode that hit the mounted CherryFlash payload while preserving the one-run dataset isolation contract.
...
### Fixed
- **Scheduling / Guide Critical Catch-Up**: the critical scheduler watchdog no longer marks `guide_excursions_full` as completed when a catch-up dispatch only materializes another resource-busy `status='skipped'` run (for example `remote_telegram_session_busy` behind a still-running `tg_monitoring` Kaggle job), so the missed daily guide slot keeps retrying on later watchdog ticks until a real `full` run materializes.
- **Guide Excursions / Critical Full-Slot Catch-Up**: the daily `guide_excursions_full` slot now participates in the same critical scheduler watchdog/startup catch-up path as other must-run jobs, and recovery only treats materialized `guide_monitoring` runs with `details.mode='full'` as delivered, so a missed full scan no longer disappears after `heavy_busy` skips just because a later `light` scan ran.
- **VK Auto Queue / Persistent Rate-Limit Deferral**: provider-side `429` in `vk_auto_import` now stores the row as `vk_inbox.status='deferred'` with a persisted retry window instead of leaving it in `locked`, so startup recovery after OOM/restart no longer requeues the same VK post immediately and create a `rate limit -> restart -> same post` loop.
- **Runtime Health / Critical Scheduler Watchdog Imports**: restored the missing `critical_scheduler_watchdog_*` imports in `main.py`, so the watchdog loop runs again and `/healthz` no longer crashes with `NameError` instead of returning a normal health report.
- **Guide Excursions / Bot Startup Router Import**: restored the missing `guide_excursions_router` import in `main.py`, so production webhook startup no longer crashes with `NameError: guide_excursions_router is not defined` during `create_app()`.
