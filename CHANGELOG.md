...
- **CherryFlash / Telegram Feed Preview**: final CherryFlash channel/test publish now sends video with `supports_streaming=True` and an explicit first-frame JPEG thumbnail, so Telegram list/feed preview no longer depends on client-side thumbnail inference for HEVC uploads.
