...
### Changed
- **CherryFlash / 2D Active Motion Compositing**: the local 2D renderers now keep poster/text `fly-in` and `fall-out` motion in subpixel space until final compositing instead of snapping active elements to integer positions too early, and the final renderer no longer hides cadence defects by starting all primary scenes from the approved first-scene late-start shortcut; local 2D probes now keep the full `zoom-in -> move-up -> exit` contract while removing near-static pairs on the previously problematic active-motion window. This remains `Not confirmed by user` until the next visual validation run.
