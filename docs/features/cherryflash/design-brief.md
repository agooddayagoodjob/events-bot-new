...
- The active `Мобильная лента` approval path now has its own Blender still tooling:
  ...
  - Kaggle preproduction runner:
    ...
    - current preproduction bootstrap stays input-first and now mirrors the Telegram/Telegraph Kaggle flow: the launcher prepares a one-run dataset with a unique slug and a mounted CherryFlash runtime tree (`mobilefeed_intro_still.py`, `scripts/`, `assets/`), waits until Kaggle reports that dataset ready and exposes the required mounted files, binds it to the kernel through `dataset_sources`, and the notebook runs directly from that mounted tree instead of cloning a branch head.
...
- CherryFlash uses the confirmed candidate-window traversal order:
  - `24 часа` first;
  - then `3 суток`;
  - then `7 суток`.
...
- Current rollout note:
  - while story autopublish is disabled, validation publications go to `@keniggpt`;
  - when story publish is enabled later, the operational readiness bar is a live story by `12:30 Europe/Kaliningrad`.
...
## Low-event follow-up beat

- When the daily base payload has `<=3` events and a selected event has a second image, the 2D scene family may extend that event with one extra follow-up beat.
- That beat keeps the same CherryFlash visual family but changes the scene grammar:
  - second image of the same event;
  - soft right-to-left `move-left` instead of the normal `move-up` emphasis;
  - lower text block switches from `title/date/location` to a short description in the `search_digest` family.
- This extra beat is a 2D scene expansion only:
  - it does **not** change intro ribbon count;
  - it does **not** create a second event in the intro;
  - it exists only to keep low-event daily releases from feeling too short or too empty.
