...
- Launcher для `CrumpleVideo` должен трактовать `ApiSaveKernelResponse` как источник истины по факту запуска, а не считать `api.kernels_push()` успешным только потому, что Python call вернулся без исключения.
- Непустой `error` в `ApiSaveKernelResponse` означает, что push не состоялся и run нельзя переводить в успешный launch state.
- Если `SaveKernel` отклонён с Kaggle weekly GPU quota error, `CrumpleVideo` должен сделать один retry по тому же `save_kernel` пути с `enable_gpu=false`, затем продолжить обычный Kaggle run на CPU при успешном retry.
- После успешного CPU fallback launcher обязан сохранить реальный `kaggle_kernel_ref` и продолжить стандартный poller/recovery path без отдельного crumple-only launcher.
...
- при публикации в Telegram stories `CrumpleVideo` не должен растягиваться или кропаться под `9:16`: notebook обязан готовить отдельную story-safe копию `1080x1920` с паддингом сверху/снизу без upscale, а сам финальный mp4 `1080x1572` при этом не меняется.
