...
- для dense multi-announce schedule cards canonical path теперь двухступенчатый: `trail_scout.screen.v1` делает только thin routing (`decision`, `extract_family`, `multi_announce_likely`, `reasons`, `confidence`), после чего `trail_scout.block_router.v1` подтверждает multi-announce и отбирает candidate blocks для block-level extraction;
...
- block-level `trail_scout.announce_extract_tier1.v1` для multi-announce path держит только flat Tier-1 fields; тяжёлые nested payloads вроде `fact_pack` / `template_hint` / `profile_hint` не должны жить в block extract schema, потому что server/runtime уже умеет восстанавливать `fact_pack` из occurrence flat fields;
- `route_weaver.enrich.v1` теперь вызывается условно: только когда после extract у occurrence всё ещё пуст `route_summary` и сам focus excerpt достаточно содержателен; короткие schedule cards не должны автоматически тратить второй LLM-call на каждый блок без явной semantic need;
...
- если scheduled/recovery `full` run импортировал результат и единственная ошибка — synthetic marker `kaggle result marked as partial`, автопубликация `new_occurrences` всё равно должна идти: soft-partial warning остаётся operator-facing, но не должен срывать весь ежедневный digest;
