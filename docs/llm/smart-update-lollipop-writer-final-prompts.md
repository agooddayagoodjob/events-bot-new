...
21. В lead и в начале каждого narrative section strongest grounded hook должен появляться раньше сухой справки; нельзя прятать самую интересную деталь в конец абзаца.
22. Для concert / list-heavy cases плохой opening — каталогическое `событие посвящено ...`; хороший opening — редкость, характер вечера, музыкальный мир или attendance-shaping promise, если это grounded в pack.
23. Report-style формулы вроде `характеризуется`, `представлены истории`, `наполнена`, `посвящен/посвящена` считаются fallback-only и должны заменяться на более естественную announcement prose.
24. Generic ensemble-category lines (`участвуют солисты, оркестр, хор ...`) не должны открывать narrative section, если в этой секции уже есть более vivid atmosphere / rarity / program hook.
25. Возрастные ограничения и прочие admin/access notes не относятся к public narrative prose и не должны становиться финальной сильной нотой текста.
26. Lead не должен открываться конструкцией `X — это ...`, если тот же смысл можно выразить через само событие, музыку, программу или сценическое действие.
27. Не допускается audience-template prose вроде `зрители смогут насладиться ...`; subject должен оставаться у события / сцены / программы, а не уходить в generic reader-address.
28. Если upstream fact text сам звучит сухо (`посвящен`, `характеризуется`, `представлены`, `наполнена`), writer обязан переписать его в более естественный event-facing русский без потери grounding.
29. Prompt должен давать не только banlist, но и positive register exemplars для целевого голоса: короткие event-led opening patterns без CTA и без report-copula.
30. При наличии grounded rarity fact он должен иметь приоритет как lead hook перед secondary credits; при наличии grounded atmosphere fact он должен появляться в первом абзаце или в opening первого narrative body block, а не в tail sentence.
31. Дополнительно запрещены формулы `приглашает вас`, `для ценителей`, `программа состоит из`, `обещает стать настоящим праздником`.
32. Retry contract обязан прямо запрещать oscillation `report -> promo` или `promo -> report`; correction pass должен переписывать тот же hook в третьем регистре: `живой сдержанный дайджест`.
...
- `посвящ...`-style dry lead opening;
- mention of poster/`афиша` in `description_md`;
- age/access restriction leakage into `description_md`;
- lead meta-opening вида `X — это ...`;
- collapse of explicit named lists into `и другие`;
- report-style formulas like `характеризуется` / `представлены истории`;
- report-style formulas like `программа состоит из`;
- promotional / CTA formulas like `не упустите шанс`, `уникальная возможность`, `не оставит равнодушным`, `приглашает вас`, `для ценителей`, `обещает стать настоящим праздником`.
- audience-template formulas like `зрители смогут насладиться ...`.
