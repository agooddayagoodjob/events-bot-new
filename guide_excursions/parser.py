...
def extract_guide_names(text: str, *, fallback_name: str | None = None) -> list[str]:
    out: list[str] = []
    patterns = [
        re.compile(r"авторская\s+экскурсия\s+([А-ЯЁ][а-яё]+(?:\s+[А-ЯЁ][а-яё]+){0,2})", re.I),
        re.compile(r"с\s+([А-ЯЁ][а-яё]+(?:\s+[А-ЯЁ][а-яё]+){0,2})", re.I),
    ]
    for pattern in patterns:
    ...
    *,
    ...
    for block in blocks:
        ...
        occurrence = GuideParsedOccurrence(
            ...
            guide_names=extract_guide_names(
                block,
                fallback_name=fallback_guide_name if allow_fallback_guide_name(source_kind) else None,
            ),
